...
def denoise4D(datacube, lam, mu, iterations=75, BC_mode=2, FISTA=False, reference_data=None):
    ...
    try:
        ...
        print("I tried to print with pretty characters but your system doesn't like Unicode...")
...
def denoise3D(datacube, lam, mu, iterations=75, BC_mode=2, FISTA=False, reference_data=None):
    ...
    print(f"Available RAM: {size(psutil.virtual_memory().available,system=alternative)}", flush=True)
    if FISTA:
        print(f"FISTA Accelerated TV denoising will require {size(datacube.nbytes*7,system=alternative)} of RAM...", flush=True)
    else:
        print(f"Unaccelerated TV denoising will require {size(datacube.nbytes*4,system=alternative)} of RAM...", flush=True)
    ...
    if FISTA:
        for i in tqdm(range(int(iterations)), desc='FISTA Accelerated TV Denoising'):
            ...
            # update accumulators
            accumulator_update_3D_FISTA(recon, acc1, d1, tk_ratio, 0, lambdaInv[0], BC_mode=BC_mode)
            accumulator_update_3D_FISTA(recon, acc2, d2, tk_ratio, 1, lambdaInv[1], BC_mode=BC_mode)
            accumulator_update_3D_FISTA(recon, acc3, d3, tk_ratio, 2, lambdaInv[2], BC_mode=BC_mode)
        ...
        for i in tqdm(range(int(iterations)), desc='Unaccelerated TV Denoising'):
            # update accumulators
            accumulator_update_3D(recon, acc1, 0, lambdaInv[0], BC_mode=BC_mode)
            accumulator_update_3D(recon, acc2, 1, lambdaInv[1], BC_mode=BC_mode)
            accumulator_update_3D(recon, acc3, 2, lambdaInv[2], BC_mode=BC_mode)
